...
class Train_Embedding_Model(object):
    ...
    def __init__(self, params, side_info, E_init, R_init, seed_pair, new_seed_triples, new_seed_sim):
        ...
        # self.p.single_batch_size = 8
        # self.p.cross_batch_size = 8
    ...
    def train(self):
        ...
        if self.p.do_train:
            ...
            train_dataloader_head = GeneratorDataset(
                source=TrainDataset(train_triples, nentity, nrelation, self.p.single_negative_sample_size, 'head-batch'),
                column_names=['positive_sample', 'negative_sample', 'subsampling_weight', 'mode'],
                shuffle=True,
                num_parallel_workers=max(1, self.p.cpu_num // 2))
            # train_dataloader_head = train_dataloader_head.batch(batch_size=self.p.single_batch_size, per_batch_map=TrainDataset.collate_fn)
            ...
            train_dataloader_tail = GeneratorDataset(
                source=TrainDataset(train_triples, nentity, nrelation, self.p.single_negative_sample_size, 'tail-batch'),
                column_names=['positive_sample', 'negative_sample', 'subsampling_weight', 'mode'],
                shuffle=True,
                num_parallel_workers=max(1, self.p.cpu_num // 2))
            ...
            optimizer = nn.optim.Adam(
                filter(lambda p: p.requires_grad, kge_model.get_parameters()),
        ...
        if self.p.init_checkpoint:
            ...
            checkpoint = mindspore.load_checkpoint(os.path.join(self.p.init_checkpoint, 'checkpoint'))
        ...
        if self.p.do_train:
            ...
            if self.p.use_cross_seed:
                ...
                seed_dataloader_head = GeneratorDataset(
                    source=SeedDataset(seed_triples, nentity, nrelation, self.p.cross_negative_sample_size, 'head-batch',seed_sim),
                    column_names=['positive_sample', 'negative_sample', 'subsampling_weight', 'seed_sim', 'mode'],
                    shuffle=True,
                    num_parallel_workers=max(1, self.p.cpu_num // 2))
                seed_dataloader_head = seed_dataloader_head.batch(batch_size=self.p.cross_batch_size)
                # seed_dataloader_head = seed_dataloader_head.batch(batch_size=self.p.cross_batch_size)

                seed_dataloader_tail = GeneratorDataset(
                    source=SeedDataset(seed_triples, nentity, nrelation, self.p.cross_negative_sample_size, 'tail-batch',
                                seed_sim),
                    column_names=['positive_sample', 'negative_sample', 'subsampling_weight', 'seed_sim', 'mode'],
                    shuffle=True,
                    num_parallel_workers=max(1, self.p.cpu_num // 2))
                seed_dataloader_tail = seed_dataloader_tail.batch(batch_size=self.p.cross_batch_size)
                ...
                if self.p.use_cross_seed:
                    if step % self.p.turn_to_seed == 0:
                        ...
                        if self.p.update_seed and step % self.p.update_seed_steps == 0 and larger and step > 0:
                            ...
                            seed_dataloader_head = GeneratorDataset(
                                source=SeedDataset(seed_triples, nentity, nrelation, self.p.cross_negative_sample_size,
                                                   'head-batch',seed_sim),
                                column_names=['positive_sample', 'negative_sample', 'subsampling_weight', 'seed_sim',
                                              'mode'],
                                shuffle=True,
        ...
        self.entity_embedding = kge_model.entity_embedding.asnumpy()
        self.relation_embedding = kge_model.relation_embedding.asnumpy()